...
class TerminalKivy(BoxLayout):
    def __init__(self, **kwargs):
        super().__init__(orientation='vertical', **kwargs)
        self.input_queue = queue.Queue() # Fila para enviar o texto do Kivy pro Jogo

        # Interface (Mesma do exemplo anterior)
        self.scroll = ScrollView(size_hint=(1, 0.9))
        self.output = Label(text="Iniciando jogo...\n", size_hint_y=None, halign='left', valign='top')
        self.output.bind(texture_size=self.output.setter('size'))
        self.scroll.add_widget(self.output)
        self.add_widget(self.scroll)

        self.input_field = TextInput(size_hint=(1, 0.1), multiline=False)
        self.input_field.bind(on_text_validate=self.enviar_pro_jogo)
        self.add_widget(self.input_field)

        # Inicia o seu jogo original em uma Thread separada
        threading.Thread(target=self.rodar_seu_jogo_original, daemon=True).start()

    def enviar_pro_jogo(self, instance):
        texto = self.input_field.text
        self.escrever_na_tela(f"> {texto}\n")
        self.input_queue.put(texto) # Coloca o texto na fila que o input() vai ler
        self.input_field.text = ""

    def escrever_na_tela(self, texto):
        # O Kivy exige que atualizações de interface aconteçam na thread principal
        Clock.schedule_once(lambda dt: self._atualizar_label(texto))

    def _atualizar_label(self, texto):
        self.output.text += str(texto)
        self.scroll.scroll_y = 0

    # --- ESSE É O SEU "ADAPTADOR" DE INPUT/PRINT ---
    def meu_input(self, prompt):
        self.escrever_na_tela(prompt)
        return self.input_queue.get() # Trava a thread do jogo até o usuário digitar no Kivy

    def meu_print(self, *args, sep=" ", end="\n", flush=False):
        texto = sep.join(map(str, args)) + end
        self.escrever_na_tela(texto)

    # --- AQUI VOCÊ COLA O SEU JOGO ---
    def rodar_seu_jogo_original(self):
        # Atalhos para não precisar renomear tudo no seu código
        print = self.meu_print
        input = self.meu_input
        def main():
            ...
            def criar_mesa_trade():
                ...
                while True:
                    ...
                    num_participantes = input("")
                    num_participantes = int(num_participantes)
                    if num_participantes > 10:
                        digitar("não é possivel criar a trade,limite ultrapassado, integrantes maximos são 10")
                    if num_participantes < 2:
                        digitar("não é possivel criar uma mesa de trades com menos de 2 pessoas")
                    else:
                        data_mesa["criador_trade"] = dados_jogador["nome"]
                        data_mesa["limite_pessoas"] = num_participantes
                        data_mesa["em_duração"] = True
                        try:
                            SERVER = "https://luizsgustavo76.pythonanywhere.com"
                            requests.post(
                                f"{SERVER}/s-trade",
